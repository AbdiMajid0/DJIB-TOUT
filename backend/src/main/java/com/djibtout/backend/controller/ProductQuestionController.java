...
@RestController @RequestMapping("/api/products/{productId}/questions") public class ProductQuestionController{
 ...
 @GetMapping public List<ProductQuestion> list(@PathVariable Long productId){return questions.findVisibleByProductId(productId);}
 @GetMapping("/mine") public ResponseEntity<?> mine(@PathVariable Long productId,Authentication authentication){var user=CurrentUser.of(users,authentication);return user==null?ResponseEntity.status(401).build():ResponseEntity.ok(questions.findByUserIdOrderByCreatedAtDesc(user.getId()));}
