...
@RestController @RequestMapping("/api/seller/team") public class SellerStaffController{
 ...
 private User owner(Authentication a){return CurrentUser.withRole(users,a,Role.SELLER,Role.ADMIN);}
 ...
 @PatchMapping("/{id}") public ResponseEntity<?> role(Authentication a,@PathVariable Long id,@RequestBody Invite input){SellerStore s=store(owner(a));SellerStaff entry=staff.findById(id).orElse(null);if(s==null||entry==null||!entry.getStore().getId().equals(s.getId()))return ResponseEntity.status(403).build();entry.setStaffRole(input.role());return ResponseEntity.ok(staff.save(entry));}
