...
public class AuthController {
 ...
 @GetMapping("/me") public ResponseEntity<?> me(org.springframework.security.core.Authentication a){User u=users.findByEmail(a.getName()).orElseThrow();return ResponseEntity.ok(Map.of("name",u.getName(),"email",u.getEmail(),"role",u.getRole().name(),"emailVerified",u.isEmailVerified()));}
 @PostMapping("/refresh") public ResponseEntity<?> refresh(@RequestBody Map<String,String> b){try{User u=refreshTokens.consume(b.get("refreshToken"));UserDetails d=userDetailsService.loadUserByUsername(u.getEmail());events.audit(u,"AUTH_REFRESH","account="+u.getId());return ResponseEntity.ok(Map.of("token",jwtUtil.generateToken(d),"refreshToken",refreshTokens.issue(u,d)));}catch(Exception e){return ResponseEntity.status(401).body(new MessageResponse("Refresh token invalide."));}}
 ...
 @GetMapping("/sessions") public ResponseEntity<?> sessions(org.springframework.security.core.Authentication a){User u=users.findByEmail(a.getName()).orElseThrow();return ResponseEntity.ok(refreshTokens.sessions(u).stream().map(t->Map.of("id",t.getId(),"createdAt",t.getCreatedAt(),"expiresAt",t.getExpiresAt(),"active",!t.isRevoked()&&t.getExpiresAt().isAfter(LocalDateTime.now(java.time.ZoneOffset.UTC)))).toList());}
 @DeleteMapping("/sessions/{id}") public ResponseEntity<?> session(org.springframework.security.core.Authentication a,@PathVariable Long id){User u=users.findByEmail(a.getName()).orElseThrow();return refreshTokens.revokeSession(u,id)?ResponseEntity.noContent().build():ResponseEntity.notFound().build();}
 @DeleteMapping("/sessions") public ResponseEntity<?> sessionsDelete(org.springframework.security.core.Authentication a){User u=users.findByEmail(a.getName()).orElseThrow();refreshTokens.revokeAll(u);return ResponseEntity.noContent().build();}
