...
export default function SellerDashboardHome(){
  ...
  return <div className="portal-content seller-dashboard-live">
    ...
    <div className="seller-dash-main"><section className="panel"><header><div><h2>Commandes récentes</h2><p>Les dernières commandes contenant vos produits.</p></div><Link to="/seller/orders">Toutes les commandes <ChevronRight/></Link></header>{orders.length?<div className="seller-order-table"><div className="head"><span>Commande</span><span>Date</span><span>Montant</span><span>Statut</span></div>{orders.slice(0,6).map(order=><div key={order.fulfillmentId}><b>#DT-{order.orderId}</b><span>{order.createdAt?dateCourte(order.createdAt):'—'}</span><strong>{money(order.subtotal)}</strong><em className={'seller-order-status '+String(order.status).toLowerCase()}>{status[order.status]||order.status}</em></div>)}</div>:<div className="seller-empty"><ClipboardList/><b>Aucune commande pour le moment</b><p>Vos nouvelles commandes apparaîtront ici.</p></div>}</section>
